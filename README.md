...
<!-- 라이브 대시보드 스크린샷: `python -m streamlit run src/inframon/dashboard/app.py` (http://localhost:8501)
     실행 후 각 탭을 캡처해 아래 경로로 넣으면 표시됩니다. -->
<!-- ![대시보드 — InSAR 탭](docs/img/dashboard_insar.png) -->
<!-- ![대시보드 — FRAM 위험 히트맵](docs/img/dashboard_fram.png) -->

> **라이브 대시보드 스크린샷 추가하는 법**: 대시보드 실행(`--serve`/`streamlit run`) 후
> 브라우저(http://localhost:8501)에서 InSAR/PINN/FRAM 탭을 캡처 → `docs/img/dashboard_*.png`
> 로 저장하고 위 주석(`<!-- -->`)을 해제하면 여기에 나타난다.
