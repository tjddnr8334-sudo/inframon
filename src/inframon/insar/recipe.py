...
def buffer_bbox(bbox: tuple[float, float, float, float], buffer_m: float
    ...
    """(min_lon,min_lat,max_lon,max_lat) 를 buffer_m [m] 만큼 사방 확장.

    교량 자체 extent 는 얇은 선이라 그대로 쓰면 AOI 가 퇴화한다. 주변에서 점을
    뽑으려면 교량 둘레로 buffer 를 줘 실제 관측 영역을 만든다.
    """
    mn_lon, mn_lat, mx_lon, mx_lat = bbox
    clat = (mn_lat + mx_lat) / 2.0
    dlat = buffer_m / 111_320.0
    dlon = buffer_m / (111_320.0 * max(0.1, math.cos(math.radians(clat))))
    return (mn_lon - dlon, mn_lat - dlat, mx_lon + dlon, mx_lat + dlat)
...
class BridgeTarget(BaseModel):
    ...
    osm_type: str
    osm_id: int
    bbox: tuple[float, float, float, float]  # (min_lon, min_lat, max_lon, max_lat) — buffer 적용된 AOI(점 추출 영역)
    aoi_buffer_m: float = 200.0              # 교량 주변 point 추출 반경 [m] (bbox 는 이만큼 확장됨)
    bridge_bbox: tuple[float, float, float, float] | None = None  # 교량 자체 extent(참고)
    ...
    @classmethod
    def from_bridge(cls, bridge: Bridge, selected_lat: float, selected_lon: float,
                    aoi_buffer_m: float = 200.0) -> BridgeTarget:
        return cls(
            name=bridge.name,
            ...
            bbox=buffer_bbox(bridge.bbox, aoi_buffer_m),   # 교량 주변 buffer = 점 추출 AOI
            aoi_buffer_m=aoi_buffer_m,
            bridge_bbox=bridge.bbox,
