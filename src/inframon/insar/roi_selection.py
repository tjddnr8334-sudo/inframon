...
def select_roi(lat: float, lon: float, *,
    ...
    후보 크기 1~10km(최소 1×1 ~ 최대 10×10). 각 크기마다 교량이 ROI 안에 남는 범위에서
    중심을 grid×grid 슬라이드해 건물수 최대 위치를 찾는다. 크기 간에는:
      · 각 후보 밀도(건물/km²) 계산 → 최고밀도의 `density_floor`(기본 0.5) 이상인 후보만
        '도심'으로 인정,
    ...
    # 도심밀도 충분한 후보 중 reference point(건물) 최대 — 없으면 전체 중 밀도 최대 폴백
    max_dens = max(c.density_per_km2 for c in cands)
    eligible = [c for c in cands if c.density_per_km2 >= max_dens * density_floor] or cands
    return max(eligible, key=lambda c: (c.n_buildings, c.size_km))
