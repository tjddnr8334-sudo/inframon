...
def filter_slaves_by_baseline(dates, master_date, *, bperp: dict | None = None,
    ...
    · 도플러 ΔfDC = |fDC_slave − fDC_master|(Hz): 크면 방위 스펙트럼 비간섭 →
      max_doppler_hz 초과 제거(S1 zero-Doppler 조향으로 대개 <50Hz). doppler 없으면 생략.
    과다제거 방지: 위반이 없는 slave 우선 유지, 최소 min_keep 미달이면 baseline 작은
    순으로 보충. 반환: (유지 날짜[정렬], 제거 [{date, reason, temporal_days, perp_m, doppler_hz}]).
    """
    from datetime import datetime
    md = datetime.strptime(str(master_date), "%Y%m%d")
    bref = (bperp or {}).get(str(master_date))
    dref = (doppler or {}).get(str(master_date))
    rows = []
    for d in dates:
        ...
        fb = (abs(doppler[str(d)] - dref)
              if doppler and str(d) in doppler and dref is not None else None)
        over = []
        ...
        if fb is not None and fb > max_doppler_hz:
            over.append(f"도플러 {fb:.0f}Hz>{max_doppler_hz:.0f}")
        score = (tb / max_temporal_days + (pb / max_perp_m if pb is not None else 0.0)
                 + (fb / max_doppler_hz if fb is not None else 0.0))
        rows.append({"date": str(d), "tb": tb, "pb": pb, "fb": fb, "over": over, "score": score})
    ...
    rejected = [{"date": r["date"], "reason": " · ".join(r["over"]),
                 "temporal_days": r["tb"],
                 "perp_m": (round(r["pb"], 1) if r["pb"] is not None else None),
                 "doppler_hz": (round(r["fb"], 1) if r["fb"] is not None else None)}
                for r in bad[i:]]
