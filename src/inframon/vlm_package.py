...
폴더 번들 `<bridge_id>_vlm/`:
  ...
  summary.json          VLM 이 바로 읽는 구조화 다이제스트(핫스팟·부재롤업·PINN·가상센싱·위험참고)
  narrative.md          템플릿 기반(LLM 아님) 자연어 서술 = VLM grounded 컨텍스트
  knowledge_graph.json  지식그래프(KG 확장점) — 타입 있는 속성 그래프(kg.py)
  figures/*.png         Vision 입력용 — VLM 은 이미지를 본다(변위맵·CRI·시계열·성분)

VLM 추론 백엔드는 `vlm/`(서브패키지) 확장점에 플러그인한다(register_backend). KG 는 kg.py
가 이 summary 를 재사용해 그래프로 변환한다 — 둘 다 향후 VLM/KG 통합의 소켓이다.
...
def build_summary(store: ProjectStore, *, bridge_id: str = "", to_crs: str = WGS84) -> dict[str, Any]:
    ...
    if pinn is not None:
        ...
        # PINN 가상센싱 전체 변위장(상부거더 1D 종축 + 상판 2D 면) — VLM/KG 가 관측점
        # 없는 위치까지 포함한 교량 상부 전체 변위량을 보게 한다.
        try:
        ...
        if vs:
            out["virtual_sensing"] = {
                "girder": {
                    "n_virtual": vs.get("n_virtual"),
                    "span_m": vs.get("span_m"),
                    "peak_total_mm": vs.get("peak_total_mm"),
                    "peak_l_from_fixed_m": vs.get("peak_l_from_fixed_m"),
                    "vertical_separated": vs.get("vertical_separated"),
                },
                "deck": vs.get("deck"),
            }
            out["channels_present"]["virtual_sensing"] = True
...
def build_narrative(summary: dict[str, Any]) -> str:
    ...
    if summary.get("virtual_sensing"):
        vs = summary["virtual_sensing"]
        g = vs.get("girder") or {}
        lines += ["", "## PINN 가상센싱 — 교량 상부 전체 변위장",
                  f"- 상부거더(종축) 전체 변위: 가상센서 {g.get('n_virtual')}개, 스팬 "
                  f"{g.get('span_m')}m, 첨두 {g.get('peak_total_mm')}mm "
                  f"@ 고정단거리 {g.get('peak_l_from_fixed_m')}m "
                  f"(연직분리={'O' if g.get('vertical_separated') else 'X'})."]
        d = vs.get("deck")
        if d:
            fp = d.get("footprint_m") or [None, None]
            lines.append(f"- 상판(2D 전체 면): 격자 {d.get('n_long')}×{d.get('n_trans')}"
                         f"(={d.get('n_deck')}점), 상판 {fp[0]}×{fp[1]}m, 첨두 "
                         f"{d.get('peak_total_mm')}mm @ {d.get('peak_xy')}. "
                         "관측점 없는 상판 위치까지 PINN 이 추정한 값.")
...
def export_vlm_package(h5_path: str | Path, out_dir: str | Path, *,
    ...
    # 지식그래프(KG) — VLM/KG 소비단이 삼킬 타입 있는 속성 그래프(가상센싱 포함).
    ...
    result = {"dir": str(out_dir), "files": files, "rows": len(rows),
              ...
              "figures": figs, "channels": summary["channels_present"],
