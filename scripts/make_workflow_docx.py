...
def build() -> Path:
    ...
    TABLE(doc, [
        ...
        ["또는", "탭 ① InSAR → ② PINN → ③ FRAM → ④ 잔존수명", "한 단계씩 보며 가려면 각 탭 **맨 위 ▶ 이 단계 실행**. 앞 단계 결과가 없으면 눌리지 않고 무엇을 먼저 할지 알려 줍니다. "
                                                     "탭 줄 아래 진행 띠에 단계별 ✅/⬜"],
    ], widths=[1.2, 3.8, 11.0])
    ...
    P(doc, "탭의 뜻: **⓪ 시작** 은 준비 확인·교량 선택·실행. **① InSAR · ② PINN · ③ FRAM · ④ 잔존수명** 은 그 순서로 만들어지는 결과를 보는 곳이고, "
           "각 탭 맨 위 ▶ 로 그 단계만 (다시) 돌릴 수도 있습니다 — ⓪ 의 ▶ 전체 실행과 같은 코드, 같은 결과 파일(<저장 폴더>\\pipeline\\project.h5). "
           "**⑤ PSI 방법론** 은 PS·SBAS·QPS 세 처리법 비교(참고).")
