...
def resolve_deck(b: Bridge) -> None:
    ...
    if b.length_m:
        fit = [c for c in cands if abs(c.length_m - b.length_m) <= 0.25 * b.length_m]
        if fit:
            road = min(fit, key=lambda c: abs(c.length_m - b.length_m))
            b.sources["deck_match"] = (f"CSV 연장 {b.length_m:.0f} m 에 맞는 way 선택"
                                       f"({len(cands)}후보 중)")
        else:
            named = [c for c in cands if c.name and b.name and b.name in c.name]
            road = max(named or cands, key=lambda c: c.length_m)
...
def residual_height(b: Bridge, sub: Path) -> dict | None:
    if not (b.proc and b.master):
        note(b, "SNAP star 처리 폴더 없음 → 잔차고도 불가(브리프 (b) 비움)")
        return None
    from inframon.insar.chainage import _signed_offset
    from inframon.insar.deck_geometry import project_to_polyline
    from inframon.insar.residual_height import run_snap_star
    with h5py.File(sub, "r") as f:
        ll = np.asarray(f["pixel_lonlat"][()], float)
    on = None
    if len(b.geometry) >= 2 and b.width_m:
        st, of = project_to_polyline(ll, b.geometry)
        of = _signed_offset(ll, b.geometry, of)
        on = np.abs(of) <= b.width_m / 2
    try:
        rh, g = run_snap_star(sub, b.proc, b.master, on_deck=on)
    except Exception as e:                       # noqa: BLE001
        note(b, f"잔차고도 실패: {e}")
        return None
...
def run_pinn(b: Bridge, tw: dict, out: Path) -> dict | None:
    ...
    prof = BridgeProfile(name=b.name, bridge_type="girder", material="concrete",
...
`{b.lat}, {b.lon}` · 트랙 `{Path(b.track).name}`
...
|---|---|---|---|---|---|
| {b.length_m:.0f} m | {b.n_spans} | {b.width_m:.1f} m | {b.clearance_m:.1f} m | {b.ground_m:.1f} m | {b.deck_az_deg if b.deck_az_deg is not None else float('nan'):.1f}° |
...
def main() -> None:
    ...
    if a.batch:
        ...
        if not (a.name and a.lat and a.lon and a.track):
            ap.error("--name --lat --lon --track 또는 --batch")
        bridges = [Bridge(a.name, a.lat, a.lon, a.track, a.proc, a.master, a.out)]
