...
def build() -> Path:
    ...
    FIG(doc, IMG / "ui_step3_plan_result.jpg", "그림 5. 계획 보기 결과 — 진행 상황에 단계별 ✅/❌. '②④ SLC·트랙·프레임 ASC path127 · 41장' 이 이 교량에 쓸 수 있는 위성 장면이다. ①이 ◐ 면 OSM 이 응답하지 않아 전국교량표준데이터(CSV) 제원으로 진행한 것 — 결과는 유효하다.")
    ...
    TABLE(doc, [
        ...
        ["OSM 조회 실패(504)", "미러 3곳 순환·자동 재시도 4회. ① 은 그래도 안 되면 전국교량표준데이터 CSV 로 진행(◐). 캐시가 있으면 캐시 사용"],
