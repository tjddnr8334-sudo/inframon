...
- ✅ **6측면 함수망**(`fram/network.py`, 설계 §5.3): 6측면(I/O/R/C/T/P) + 기능 결합 토폴로지(열팽창→받침 등) DiGraph, R_ij 가중 → 기능별 criticality·driver·임계 전파경로·응집도. networkx 선택(numpy 폴백). `/fram` `function_network` attr.
- ✅ **R_div 예측 발산 재정의**: |2차미분| → **롤링 선형예측 잔차**(`_forecast_residual`, 설계 `‖d_obs−d_linear(t)‖`). 선형≈0·비선형 가속이면 큼. onset 에서 발산 감지.
- ✅ **FastAPI 실시간 서빙**(`serve.py`, CLI `--serve`): `/health`·`/status`·`/cri`·`/function-network`. project.h5 매 요청 재읽기. fastapi 선택 의존(`.[serve]`).
- ✅ **Prefect 스케줄링**(`schedule.py`, CLI `--schedule SECONDS`): `monitor_cycle`(새 Track 인제스트→PINN+FRAM 재계산→경보 **에스컬레이션**)을 interval 스케줄·재시도로. prefect 선택 의존(`.[schedule]`). → **로드맵 G5 항목 전부 구현**.
