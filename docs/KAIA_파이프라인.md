...
| **VLM 입력 패키지** | manifest·csv·summary·narrative·figures 번들 | ✅ | `--export-vlm <dir> [--zip] [--no-figures]` |
...
| `summary.json` | 구조화 다이제스트 — 변위통계·핫스팟·부재롤업·PINN(EI/저강성/고유진동수)·위험참고 |
...
- **구축 미완(우리 범위 밖)**: 시방서 RAG 평가·지식그래프·보수보강 추천은 **VLM 팀 영역**.
