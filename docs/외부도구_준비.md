# 외부 도구 준비 — Earthdata 토큰 · SNAP · snaphu · SLC 폴더

위성 원본(SLC)부터 돌리려면 이 셋이 필요합니다. `doctor` 의 `[외부 도구·자격]` 세 줄이 그것입니다.
거기에 **SLC 보관 폴더**(원본이 떨어질 큰 드라이브)를 하나 더 정합니다.
...
| **SLC 보관 폴더** | 위성 원본이 떨어지는 폴더 (예: `E:\SLC`) | 장당 4–8 GB, 교량 하나에 200–400 GB — C: 에 두면 곧 찬다. 한 번 받은 장면은 다음 교량에서 재사용 | 드라이브 고르기 (프로그램이 폴더를 만듦) |
...
| **C. 한 줄 설치기** | 먼저 `cd E:\` (프로그램을 둘 곳) → `irm https://raw.githubusercontent.com/tjddnr8334-sudo/inframon/main/install.ps1 \| iex` | 새 컴퓨터. 그 폴더 아래 `inframon` 에 받고 B 를 포함 |
...
5. 토큰은 비밀번호와 같습니다 — **터미널(또는 대시보드 칸)에만** 붙여넣고, 채팅·메일·문서에는 넣지 마세요. 어딘가에 노출됐으면 같은 페이지에서 새로 발급받고(최대 2개) 옛것은 **Revoke** 로 지웁니다.
...
**확인**: `.venv\Scripts\python -m inframon --doctor` →  `✅ Earthdata 토큰   C:\Users\…\.inframon\earthdata_token (만료 D-59)`
...
**확인**: `.venv\Scripts\python -m inframon --doctor` → `✅ SNAP gpt   C:\Users\…\esa-snap\bin\gpt.exe`
...
**확인**: `.venv\Scripts\python -m inframon --doctor` → `✅ snaphu   wsl:/usr/bin/snaphu`
...
## 4. SLC 보관 폴더 — 원하는 드라이브에

### 4-1. 대시보드 (권장)

🔧 패널 맨 아래 **SLC 보관 폴더** — 이미 설정돼 있어도 항상 보입니다 (드라이브가 차면 옮기라고).

1. **드라이브** 드롭다운 — 여유 공간이 큰 순으로 나옵니다 (`E:\ 여유 1827 GB` 처럼).
2. **폴더** — 고른 드라이브의 `\SLC` 가 자동으로 채워집니다. 다른 이름을 원하면 고칩니다.
3. **폴더 만들고 저장** — 없는 폴더는 만들고 `~/.inframon/config.json` 의 `slc_dir` 에 기록.

### 4-2. 터미널

```powershell
python -m inframon --slc-dir E:\SLC        # 없으면 만든다
```

`python start.py --tools` 도 미설정이면 드라이브별 여유 공간을 보여 주고 묻습니다 (Enter = 여유가 가장 큰 드라이브의 `\SLC`).

### 4-3. 그 뒤에 일어나는 일

- 새 다운로드가 `E:\SLC\ASC_path127_frame120\` 처럼 **궤도·프레임별 하위 폴더**에 떨어집니다. 처리용 프로젝트 폴더(`…\pipeline\SLC\`)에는 하드링크만 생깁니다 (같은 드라이브면 디스크 추가 사용 0, 다른 드라이브면 복사).
- 같은 프레임을 쓰는 다음 교량은 **다운로드를 건너뜁니다** (`보관폴더 재사용: n장` 으로 표시).
- 이미 받아둔 SLC zip 이 있으면 그 폴더(하위 폴더 포함)를 지정하기만 하면 인식합니다 — `인식된 SLC: 73장`.
- 환경변수 `INFRAMON_SLC_DIR` 이 config 보다 우선합니다.

### 4-4. 알아둘 것

- **프로젝트 저장 폴더(대시보드 왼쪽 '저장 위치')도 같은 드라이브**에 두면 하드링크가 돼 복사본이 생기지 않습니다.
- 외장 드라이브도 됩니다. 뽑혀 있으면 `get_slc_dir` 가 None 을 돌려주고 다운로드는 프로젝트 폴더로 갑니다 (경고).
- NAS(네트워크 드라이브)는 하드링크가 안 돼 복사됩니다 — 됩니다만 2배 공간.
