...
# 3-4b. 지상 실측/FEM 기준점으로 InSAR 변위 검증 (기준 CSV 를 직접 줄 때)
python -m inframon --validate data/project_real.h5,ref.csv --validate-tol 1.0
#   ref.csv = lon,lat,value  (value = 속도 mm/yr 또는 변위 mm)
#   측량좌표(중부원점 등)로 받았으면:  --validate-origin 중부원점  (x,y,value 헤더 OK)
#   → InSAR 는 상대변위·실측은 절대값이라 공통 오프셋(프레임차)을 자동 제거하고
#     그 잔차(bias제거 RMSE)로 판정한다. bias 는 오차가 아니라 프레임차라 따로 보고.

# 3-4c. 수준측량 성과표(회차별 표고 m)로 검증 — 성과표를 바로 넣을 때
python -m inframon --validate-leveling data/jeongjagyo_real.h5,성과표.csv \
  --leveling-origin 중부원점 --validate-tol 1.0 --validate-dist 15
#   성과표.csv(wide): 측점,X,Y,20200510,20210512,...  (X,Y=측량좌표, 각 날짜열=표고 m)
#   날짜 헤더가 아니면(1차,2차):  --leveling-dates 2020-05-10,2021-05-12,...
#   → 표고→연직 침하속도[mm/yr] 로버스트 회귀 + 측량좌표→WGS84 + 입사각 LOS 투영
#     + 프레임오프셋 제거까지 자동. 침하=음수(표고 하강).
...
### 6-A. 이름으로 (대시보드 ① 탭 또는 사이드바 검색)

전국교량표준데이터·OSM 에서 교량명으로 찾는다. **이름 있는 교량**에 적합.

### 6-B. 좌표로 — 이름 없는 중소형 교량

이 프로그램의 주 타겟은 **이름 없는 중소형 교량**이고, 그건 검색으로 안 나온다. 대신
설계도서·대장의 **측량 좌표**(한국 원점계)로 위치를 지정한다.

```bash
# 한국 측량 좌표 → WGS84 경위도·지도 링크
python -m inframon --locate 209655,529915 --origin 중부원점
#   → WGS84 37.3685, 127.109 + OSM 지도 링크
```

- **원점계**: `중부원점`(EPSG:5186·서울·경기·충청·전북·전남·경남, 가장 흔함) ·
  `서부원점`(5185·인천) · `동부원점`(5187·강원 동부) · `동해원점`(5188·울릉). EPSG 코드나
  `동부` 같은 약칭도 된다.
- **X=동거리(Easting), Y=북거리(Northing)** 순서. 바꿔 넣으면 한국 밖으로 떨어져 경고한다.
- 대시보드 사이드바 **📐 좌표로 지정** 에서 원점계를 고르고 X·Y 를 넣으면 위치가 나오고,
  **이 좌표로 교량 설정** 버튼으로 레시피가 만들어진다.

> ⚠️ **원점계는 반드시 설계도서·대장에 적힌 그대로 고른다.** 한국 TM 은 경도원점만 다른
> 같은 수식이라, 같은 X,Y 를 중부로 읽으면 127°, 동부로 읽으면 129°가 나온다 —
> **좌표만으로는 원점계를 구분할 수 없다.** 명백한 오인(한국 밖·관할 크게 벗어남)은
> 경고하지만, 인접 원점계 모호성은 지도 링크로 눈으로 확인해야 한다.
