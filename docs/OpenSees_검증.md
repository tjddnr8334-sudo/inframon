...
## 깊은 보 진동수: Timoshenko 보정 (2026-07)

Tier 1 이 찾은 전단 모델오차(E-B 가 깊은 보 진동수 과대예측)를 보정했다. `_timoshenko_factors`
가 E-B 진동수에 표준 보정계수 `1/√(1+(nπr/L)²(1+E/κG))`(r²=I/A, κ=5/6, G=E/2(1+ν))를
곱한다. OpenSees 완전 Timoshenko(전단+회전관성, `rotary=True`) 정해 대비 검증:
...
슬렌더 보에선 ≈1 로 무해하고, 깊은 보에서 오차를 ~5× 줄인다. 단면(폭·높이) 미상이면 보정
없이 1.0(안전). 실제 교량 대부분이 슬렌더라 영향은 작지만, 3종·짧은 박스거더엔 의미 있다.
